...
def detect_attacks(pcap_file, model_path='network_attack_model', scaler_path='network_attack_scaler.pkl', output_dir='./'):
                    ...
                    if mac and mac != "00:00:00:00:00:00":
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                print(f"Successfully blocked malicious source: {ip} (MAC: {mac})")
                            else:
                                print(f"Failed to block source {ip}. Status code: {response.status_code}")
                                print(f"Response: {response.text}")
                        except Exception as e:
                            print(f"Error blocking malicious source {ip}: {e}")
            else:
                print("No MAC addresses found in attack packets. Blocking by IP only.")
                for ip in attack_packets['src_ip'].unique():
                    try:
                        # Generate a MAC from IP (this is just a simple mapping for demonstration)
                        ip_octets = ip.split('.')
                        mac = f"00:{ip_octets[1]}:{ip_octets[2]}:{ip_octets[3]}:00:00"
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"Attack packets detected: {len(attack_packets)}\n\n")
        ...
        if not attack_packets.empty and len(attack_sources) > 0:
            f.write("\nTop Attack Sources:\n")
            for ip, count in attack_sources.items():
